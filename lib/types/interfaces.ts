...
export interface iSettings {
  ...
  existingSubnetIDs: Array<{ ID: string; AZ: string; type: string }>;
  existingRDS?: boolean;
  ...
  Route53ResolverConfig: {
    ips: Array<CfnResolverEndpoint.IpAddressRequestProperty>
  },
  domainName: string;
...
export interface ContainerProps extends StackProps {
  ...
  branch: string;
  buildArgs: string[];
  buildInstallCommands?: string[] | null;
  variables: any;
  port: number;
  memory: number;
  cluster: ICluster;
